...
# 新增 BaselineAdv 类（支持对抗性风格对齐）
class BaselineAdv(baseline):
    def __init__(self, neck=None, feature_exchange_type='le'):
        super().__init__(neck, feature_exchange_type)
        ...
        # 添加域鉴别器（为每个尺度特征定义一个鉴别器）
        self.discriminators = nn.ModuleList([
            DomainDiscriminator(in_channels=c) for c in actual_channels
        ])
...
if __name__ == '__main__':
    ...
    change_maps, z_A, z_B = model(xA, xB)
    print(change_maps[0].shape, change_maps[1].shape)  # Should print torch.Size([2, 2, 256, 256])
    print([z.shape for z in z_A], [z.shape for z in z_B])  # Print feature shapes
