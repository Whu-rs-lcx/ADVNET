import torch.nn
from change_detection.CWCD.MFRB.arch_util import *
from change_detection.CWCD.MFRB.ffc import *
from change_detection.CWCD.MFRB.wtconv.wtconv2d import *
...
class ProcessBlock(nn.Module):
    def __init__(self, dims, d_state=16, n_l_block=1, n_h_block=1, LayerNorm_type='WithBias'):
        ...
        # 替换 Mamba 为卷积块
        self.hhconv = nn.ModuleList()
        self.norm2 = LayerNorm(self.dim, LayerNorm_type)
        for layer in range(self.hnum):
            self.hhconv.append(nn.Sequential(
                nn.Conv2d(self.dim, self.dim, kernel_size=3, stride=1, padding=1, bias=True),
    ...
    def forward(self, x):
        ...
        # 替换 Mamba 处理为卷积块处理
        for conv_block in self.hhconv:
            e_high = conv_block(e_high) + e_high
