...
def CM2MetricTorch(confu_mat_total: torch.Tensor, eps: float = 1e-4):
    """
    从二分类或多分类的混淆矩阵（shape=[C, C]）计算 OverallAccuracy, MeanF1, MeanIoU, Kappa,
    以及每类的 IoU, F1, Recall, Precision。
    
    参数:
        confu_mat_total: torch.Tensor, 混淆矩阵，shape=(C, C)
        eps: float, 防止除零的小常数（默认为1e-4，对应原代码的+0.0001）
    
    返回:
        oa: Overall Accuracy (标量)
        mean_f1: Mean F1-score (标量)
        mean_iou: Mean IoU (标量)
        kappa: Kappa 系数 (标量)
        class_iou: 每类 IoU, shape=(C,)
        class_f1: 每类 F1-score, shape=(C,)
        class_recall: 每类 Recall, shape=(C,)
        class_precision: 每类 Precision, shape=(C,)
    """
    # 转为 float 并加 eps
    cm = confu_mat_total.to(torch.float32) + eps
    C = cm.shape[0]

    # 行和、列和、总和
    row_sum = cm.sum(dim=1)      # 每行之和：真实样本数
    col_sum = cm.sum(dim=0)      # 每列之和：预测样本数
    total   = cm.sum()           # 混淆矩阵总和

    # Overall Accuracy
    oa = torch.diagonal(cm).sum() / total

    # Kappa
    pe_fz = (row_sum * col_sum).sum()
    pe = pe_fz / (total * total)
    kappa = (oa - pe) / (1 - pe + eps)

    # 各类 TP、FN、FP
    tp = torch.diagonal(cm)                # 真阳性
    fn = row_sum - tp                      # 假阴性
    fp = col_sum - tp                      # 假阳性

    # 每类 F1 和 IoU
    class_f1 = (2 * tp) / (2 * tp + fp + fn + eps)
    class_iou = tp / (tp + fp + fn + eps)

    # Mean F1, Mean IoU
    mean_f1 = class_f1.mean()
    mean_iou = class_iou.mean()

    # 每类 Precision, Recall
    class_precision = tp / (col_sum + eps)
    class_recall    = tp / (row_sum + eps)

    # 四舍五入到 8 位小数（可选）
    keep = 8
    factor = 10 ** keep
    class_f1       = torch.round(class_f1       * factor) / factor
    class_iou      = torch.round(class_iou      * factor) / factor
    class_precision = torch.round(class_precision * factor) / factor
    class_recall    = torch.round(class_recall    * factor) / factor

    # 关闭科学计数法显示（可选）
    torch.set_printoptions(sci_mode=False)

    return oa, mean_f1, mean_iou, kappa, class_iou, class_f1, class_recall, class_precision
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pppred', type=str, default='/home/jic2/dsj_code/daily_code/24/rscd/test_result')
    parser.add_argument('--gggt', type=str, default='/media/jic2/HDD/DSJJ/CDdata/CLCD/test/label')
